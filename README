This one actually includes other libraries (SDL2_gfx, SDL2_image, SDL2_mixer and SDL2_ttf). They might be out of date but it works.
...
Then to install
```
nuget add .\Reyuu.SDL2-CS.1.0.0.nupkg -Source "C:\\Program Files (x86)\\Microsoft SDKs\\NuGetPackages"
```
If your configuration stores the packages somewhere else, update the `Source` argument accordingly.

The native DLLs are taken directly from SDL repositories, I've included licenses for transparency.
